...
use App\Library\JackApi\eveApi\eveDb;
use App\Library\JackApi\eveApi;
use App\Library\JackApi\eve;
use App\Library\JackApi\pages;
use App\Library\JackApi\auditorSkillsPage;
...
class JackLib{
	private $apiKey,$vCode,$charID,$listType;
	public function __construct($apiKey,$vCode,$charID,$listType=""){
		$this->apiKey = $apiKey;
		$this->vCode = $vCode;
		$this->charID = $charID;
...
elseif (isset($short_api_key)&&!$charSelect)
	...
	$urlAuthInfo = (isset($chid) ? "chid=$chid&" :"") . "usid=" . USER_ID . "&apik=" . API_KEY . "&small";
...
if (PAGE_VIEW == "onepage") {
	...
	if ($error) {
		...
		if ($page->Header != "") {
			$returnOutput .=  $page->Header . "<br>";
		} else 
			$returnOutput .= "<span style=\"font-size:450%\">". USER_NAME . "</span><br>";
	
		$returnOutput .= $page->Output . "<br>";
		//echo $page->Times . "<br>";
		$returnOutput .= "</body></html>";
		return $returnOutput;
	}
}	
